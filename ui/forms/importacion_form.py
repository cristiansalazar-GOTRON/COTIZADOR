...
class ImportacionForm(BaseCotizacionForm):
    titulo = "Datos de importacion"

    def __init__(self, parent=None) -> None:
        super().__init__(parent)

        self.precio = self.create_spinbox()
        self.moneda = self.create_combo(["COP", "USD", "EUR"])
        self.tasa_usd = self.create_spinbox()
        self.tasa_eur = self.create_spinbox()
        self.flete = self.create_spinbox()
        ...
        self.add_field(0, "Precio", self.precio, 0)
        self.add_field(0, "Moneda", self.moneda, 1)
        self.add_field(1, "Tasa USD", self.tasa_usd, 0)
        self.add_field(1, "Tasa EUR", self.tasa_eur, 1)
        self.add_field(2, "Flete", self.flete, 0)
        self.add_field(2, "Tipo flete", self.flete_modo, 1)
        self.add_field(3, "Peso", self.peso, 0)
        self.add_field(3, "Ganancia", self.ganancia, 1)
        self.add_field(4, "IVA", self.iva, 0)
    ...
    def _populate(self, values: dict[str, Any]) -> None:
        self.precio.setValue(float(values.get("precio", 0)))
        ...
        self.flete.setValue(float(values.get("flete", 0)))
